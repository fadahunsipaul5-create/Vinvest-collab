...
const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post<RegisterResponse>(`${baseUrl}/users/register/`, formData);
  ...
  const handleGoogleLoginSuccess = async (credentialResponse: any) => {
    try {
      ...
      const response = await axios.post<GoogleAuthResponse>(`${baseUrl}/users/google-auth/`, {
        token: token,
      });
